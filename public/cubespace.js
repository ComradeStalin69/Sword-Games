...
var camX = 0, camY = 0, maxSpeed = 0.1;
...
// Update stuff
function update(t) {
	camera.position.z += speed * t * (1 + (0.15*Math.floor(camera.position.z / transitionLength)));
...
function cube(x, y, z, mat) {
	var cube = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 0.7), mat);
...
function cubeGen(t, mat) {
	...
	for(i = 0; i < Math.floor(t_rem / spawn_inter); i++)
	{
		var c = new cube(camera.position.x + randPosition(), camera.position.y + randPosition(),
					     camera.position.z + (Math.random() * 0.06 * t) + 30, mat);
		scene.add(c);
	}
